...
@Log4j2
class HiperBootRepositoryTest extends BaseTestClass {

    @Autowired
    private ParentTableHiperBootRepository level01Repository;

    private List<String> columnsGreaterThanIncompatible = List.of("colBoolean", "colStatusEnum");
    private List<String> columnsInIncompatible = List.of("colBoolean");
...
//            System.out.printf("columnName %s  /  val1 %s  / val2 %s \n", columnName, val1, val2);
            results = level01Repository.getByHiperBootFilter(ParentTable.class, columnEqualValues(columnName, val1.toString(), val2.toString()));
            assertThat(results).isNotEmpty();
...
//                System.out.printf("columnName %s  /  val1 %s  / val2 %s / Result %s\n", columnName, val1, val2, valResult);
    ...
    void randomBetweenFilterAllColumnsTest() {
        ...
        for (Map.Entry<String, Field> entry : fieldMaps.entrySet()) {
            ...
            results = level01Repository.getByHiperBootFilter(ParentTable.class, columnBetween(columnName, valFrom.toString(), valTo.toString()));
            assertThat(results).isNotEmpty();
            ...
            for (ParentTable row : results) {
                ...
                System.out.printf("columnName %s  /  valFrom %s  / valResult %s \n", columnName, valFrom, valResult);
                if (columnName.equals("colString")) {
                    assertThat((Comparable) valResult.toString().toUpperCase()).isBetween(valFrom.toString().toUpperCase(), valTo.toString().toUpperCase());
                  }
                else {
    ...
    protected ParentTable getRandomRow() {
        ...
        var randomRow = results.get(randomInt);
